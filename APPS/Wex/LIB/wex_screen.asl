...
////////////////  WOBS ///////////////
  ...
  calwo=cWo(vp,_WGRAPH,

   sWo(calwo,_WNAME,"CAL",_WVALUE,0,_WCLIPBORDER,BLACK_) ; // cals;

  carbwo=cWo(vp,_WGRAPH,_WNAME,"CARB",_WVALUE,0,_WCLIPBORDER,BLACK_) ; // carbs;

  extwo=cWo(vp,_WGRAPH,_WNAME,"XT",_WVALUE,0,_WCLIPBORDER,BLACK_,_WEO); // exercise time;

  int wedwo[] = { gwo, calwo,  carbwo, extwo,-1  };
//<<[_DB]"%V$wedwo \n"
    // vtile before set clip!

  wovtile(wedwo,0.1,0.08,0.99,0.99,0.01)   ; // vertically tile the drawing areas into the main window;
...
//<<"after proc - next statement missed? \n" // MUST FIX
  ...
  ok=sWo(wedwo,_WCLIP,CXY,_WCOLOR,LILAC_,_WCLIPBHUE,WHITE_,_WBHUE,WHITE_,_WFONT,F_SMALL_,_WSAVE,_WSAVEPIXMAP,_WEO);
  ...
  sWo(wedwo,_WBORDER,_WCLIPBORDER,BLACK_,_WDRAWON,_WEO);
...
//<<"SCALES %V$sc_startday $sc_end \n"
 ...
 sWo(swo,_WNAME,"BenchPress",_WCOLOR,"white",_WEO);

  int xwo[] = { swo };

  wo_vtile(xwo,0.01,0.05,0.97,0.97,1)   ; // vertically tile the drawing areas into the main window;

  sWo(xwo,_WCLIP,CXY,_WCOLOR,WHITE_,_WCLIPBORDER,BLACK_,_WEO);
///  measurement

  tw_wo= cWo(gwo,_WSYMBOL);
  sWo(tw_wo,_WRESIZE,wbox(0.1,0.1,0.15,0.25,0),_WNAME,"TW",_WVALUE,"175",_WEO);

  sWo(tw_wo,_WVHMOVE,ON_,_WSYMBOL,STARDAVID_,_WPIXMAPON,_WREDRAW,_WEO);
//////////////////////////// SCALES //////////////////////////////////////////
//<<[_DB]" Days $k \n"
...
//sleep(0.2)
///////////////////////////////////////////////////////////
  //quitwo=cWo(vp,_WBN,_WNAME,"QUIT",_WCOLOR,"red")

  zinwo=cWo(vp,_WBN)
  sWo(zinwo,_WNAME,"ZIN",_WCOLOR,PINK_,_WHELP," zoom in on selected days ",_WEO);

  zoomwo=cWo(vp,_WBN,_WNAME,"ZOUT",_WCOLOR,"cadetblue",_WEO);
 // yrdecwo= cWo(vp,_WBN,_WNAME,"YRD",_WCOLOR,"violetred",_WHELP," show previous Year  ")
//  yrincwo= cWo(vp,_WBN,_WNAME,"YRI",_WCOLOR,"purple",_WHELP," show next Year  ")
//  qrtdwo=  cWo(vp,_WBN,_WNAME,"QRTD",_WCOLOR,"violetred",_WHELP," show previous Qtr period ")
//  qrtiwo=  cWo(vp,_WBN,_WNAME,"QRTI",_WCOLOR,"purple",_WHELP," show next Qtr period ")
 // int fewos[] = {zinwo,zoomwo, yrdecwo, yrincwo, qrtdwo, qrtiwo }

  int fewos[] = {zinwo,zoomwo,  };

  wohtile( fewos, 0.03,0.01,0.43,0.07,0.05,3);

  nobswo= cWo(vp,_WBV,_WNAME,"Nobs",_WCOLOR,"Cyan",_WVALUE,0,_WEO);

  xtwo= cWo(vp,_WBV,_WNAME,"xTime",_WCOLOR,"violetred",_WVALUE,0,_WEO);

  xbwo= cWo(vp,_WBV,_WNAME,"xBurn",_WCOLOR,"violetred",_WVALUE,0,_WEO);

  xlbswo= cWo(vp,_WBV,_WNAME,"xLbs",_WCOLOR,"violetred",_WVALUE,0,_WEO);

  dlbswo= cWo(vp,_WBV,_WNAME,"dLbs",_WCOLOR,YELLOW_,_WVALUE,0,_WEO);

  int xwos[] = { nobswo, xtwo, xbwo, xlbswo, dlbswo ,-1};

  wohtile( xwos, 0.45,0.01,0.83,0.07,0.05,5);
  ...
  xtmwo=cWo(vp,_WBV,_WNAME,"EXT",_WCOLOR,GREEN_,_WHELP," xtime on day ",_WEO);

  int mwos[] = { dtmwo, obswo, wtmwo, cbmwo, xtmwo,-1};

  wovtile( mwos, 0.02,0.5,0.08,0.9,0.05);
  for (int j= 0; mwos[j]>0; j++)
     sWo(mwos[j],_WSTYLE,"SVB",_WREDRAW,_WEO);
//  Goal WOBS
  ...
  wovtile( goalwos, 0.02,0.1,0.08,0.45,0.05);
 for (int j= 0; goalwos[j]>0; j++)
  sWo(goalwos[j],_WSTYLE,"SVB",_WREDRAW,_WEO);
