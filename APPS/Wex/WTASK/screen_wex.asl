...
//<<"%V $vptitle $vp \n"
  ...
  //sWi(_WOID,vp,_WCLIP,wbox(0.01,0.01,0.90,0.99),_WPIXMAP,ON_,  _WREDRAW,ON_,_WSAVE,ON_,_WSAVEPIXMAP,ON_);

  sWi(_WOID,vp,_WCLIP,wbox(0.01,0.01,0.90,0.99,0),  _WREDRAW,ON_,_WSAVE,ON_);
...
//titleButtonsQRD(vp1);
 ...
 // titleButtonsQRD(vp);
  ...
  sWi(_WOID,vp,_WDRAW,ON_,_WSAVE,ON_,_WBHUE,WHITE_);

  sWi(_WOID,vp1,_WDRAW,ON_,_WSAVE,ON_,_WBHUE,WHITE_);
...
//  sWo(wedwos,_WCLIP,wbox(CXY),_WCOLOR,RED_,_WCLIPBHUE,ORANGE_,_WBHUE,LILAC_,_WFONT, F_SMALL_,_WSAVE,_WSAVEPIXMAP,_WEO);
  ...
  sWo(_WOID,extwo,_WSCALES,wbox(sc_startday,10.0,sc_end,300.0,0),_WSAVESCALES,0);
...
//  sWo(extwo,_WSAVESCALES,0,_WEO);
  ...
  sWo(_WOID,calwo,_WSCALES,wbox(sc_startday,0,sc_end,CalsY1,0),_WSAVESCALES,0);
...
//<<[_DB]" Days $k \n"
  ...
  sWo(_WOID,wtwo,_WSCALES,wbox(sc_startday,155.0,sc_end,250.0));
...
<<"%V $krx $kry $kcalx $kcalY \n"
 ...
 //<<"%V  $(kcalx +0.02) $(kry +0.01) $(kcalx +0.09) $kcalY \n"
   ...
   sWo(_WOID,keycalwo,_WRESIZE,wbox(kcalx+0.02,kry+0.01,kcalx+0.09,kcalY,0),_WCLIPBORDER,PINK_,_WREDRAW,ON_);
