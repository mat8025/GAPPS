...
//<<[2]"setting %V$initial_alt \n"
    comment=scat(comment,"alt $initial_alt ")
      k++
    } 
    else
       k = 1  //reset args
...
comment=scat(comment,"climb ${c_time}s rate $c_rate ")
    k++
    }
    else if ( wc == 'l') {
...
//<<"found level \n"
          FM[n_fp] = LEVEL
    c_time = atoi(_clarg[k]) 
comment=scat(comment,"level ${c_time}s ")
          k++
    c_rate = 0
    }
    else  { 
//<<"found descend \n"
    ...
    k++
    c_rate = atoi(_clarg[k]) 
comment=scat(comment,"descend ${c_time}s rate $c_rate ")
    k++
    }
...
//      A[fp_s:fp_e:1] = vgen(FLOAT,ns+1,ca,cr) 
      ...
      A[fp_s:fp_e:1] = sgen(FLOAT,ns+2, air, ca) 
