...
float Climb_mode_threshold = 0.1
...
<<"%V$i $hold_mode $hold_descent_mode $hold_climb_mode $tim $delta_baro\n"
...
<<[2]"updating %V$hold_descent_flight_phase to $flight_phase\n"
          ...
          // we are level  less than 100 climb/descent rate
      ...
      if (slew_to_level_mode > 0) {
        ...
        if (flight_mode == DESCENT) {
            ...
            if (slew_flight_phase < flight_phase) {
                  ...
                  FPfilter = (slew_flight_phase * TCR) // reset FPfilter by vector mul
        ...
        if (slewing_from_descent) {
        slew_flight_phase = slew_descent_flight_phase * (1 - (slew_cnt - slew_to_level_mode)/(slew_cnt * 1.0)) 
        }
        if (slewing_from_climb) {
        slew_flight_phase = slew_climb_flight_phase * (1 - (slew_cnt - slew_to_level_mode)/(slew_cnt * 1.0)) 
        }
...
//<<[2]"%V$slew_to_level_mode $flight_phase\n"
        ...
        slew_to_level_mode -= dsecs


        if (slew_to_level_mode == 0) {
             // regardless of whether we were climbing now flush long-term average buffer 
             // so flight-phase will be zero
             FPfilter = 0  // vector set
             slewing_from_climb = 0
             slewing_from_descent = 0
             hold_climb_flight_phase = 0.0;
             hold_descent_flight_phase = 0.0;
             slew_descent_flight_phase = 0.0;
             slew_climb_flight_phase = 0.0;
        }
