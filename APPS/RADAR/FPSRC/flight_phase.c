...
**
** $Id: flight_phase.c 468 2010-04-14 18:14:56Z amterry $
**----------------------------------------------------------------------------
*/
 ...
 void initFlightPhase( FLIGHT_PHASE_TYPE *flightP)
{
 int i;
     ...
     flightP->flight_phase = 0.0;
      ...
      flightP->min_descent_phase_level = -0.5;  // above this no descent hold
      flightP->min_climb_phase_level = 0.5;  // below this no climb hold
...
int fmode = LEVEL;
         ...
         if (flightP->delta_baro > 0 && flightP->last_delta_baro > 0) { // we have a climb
             fmode = CLIMB;
         }
         else if (flightP->delta_baro < 0 && flightP->last_delta_baro < 0) {  // definite descent
             fmode = DESCENT;
...
// after hold is over -- then flight-phase is commanded by flight climb/descent rate
               switch (flightP->flight_mode) {
                   ...
                   case DESCENT:
                     ...
                     if ( flightP->hold_descent_mode_secs > 0) {  // level ---> descent
		   ...
		   //    printf("new  DESCENT during hold set fphase to hold_flight_phase %6.4f  \n",flightP->hold_flight_phase);
                  ...
                  case CLIMB:
                     ...
                     if (flightP->hold_descent_mode_secs > 0) {   // descent ---> climb
                   ...
                   //     if (flightP->flight_phase == -1.0) flightP->f_rate = 2.0;
                        ...
                        if (flightP->hold_descent_mode_secs <= 0) {      // now transition to slew_to_level_mode
                         flightP->hold_mode = 0;
                       }
                     }
                  ...
                  case LEVEL:
                     ...
                     if (flightP->hold_descent_mode_secs > 0) {  // descent hold ---> level
                    ...
                    // if (flightP->flight_phase == -1.0) flightP->f_rate = 2.0;
                       ...
                       flightP->hold_descent_mode_secs -= dsecs;
                //       flightP->flight_phase = flightP->hold_flight_phase;  
                // hold it at the last descent_flight_phase -- until hold mins is up
             ...
             //          printf("in hold_descent_mode %d  set flight_phase %6.4f\n",flightP->hold_descent_mode_secs, flightP->flight_phase);
                      ...
                      if (flightP->hold_climb_mode_secs > 0) {  // climb hold ---> level
                        // level and holding
                      ...
                      //  if (flightP->flight_phase == 1.0) flightP->f_rate = 2.0;
                      //   flightP->flight_phase = flightP->hold_flight_phase;   
                      // hold it at the last hold_flight_phase -- until hold mins are up

                        if (flightP->hold_climb_mode_secs <= 0) {      // now transition to slew_to_level_mode
                         flightP->hold_flight_phase = 0.0;
                         flightP->hold_mode = 0;
                       }
...
void logFlightPhase (FLIGHT_PHASE_TYPE *FlightP, int state)
{
       ...
       switch (state) {
        ...
        case 1:
fprintf(logfile,"%d,%d,%d,%d,%d,%d,%6.0f,%6.0f,%6.2f,%6.2f,%6.2f,%6.2f,%6.2f,%6.2f,%6.2f\n",
	FlightP->fp_cnt, FlightP->tim, FlightP->flight_mode, 
        FlightP->hold_mode, FlightP->hold_descent_mode_secs, FlightP->hold_climb_mode_secs,
...
radio_fp(FLIGHT_PHASE_TYPE *FlightP)
{
    ...
    elapsed_time = FlightP->tim;
                  ...
                  {                       
                     ...
                     else if (delta_baro > 250.0)
                     {
                     	  /* if still in descent hold, wait for time-out */
                        if (fp_mode == DESCENT_HOLD)
                        {
                           if ((elapsed_time - start_hold_time) >= 300.0)
                           { 
                              fp_mode = ASCENT;
                           }
                        }
                        else if ((fp_mode == DESCENT) && (flight_phase < 0.0))
                        {
