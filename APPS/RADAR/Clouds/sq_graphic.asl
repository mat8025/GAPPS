...
//<<"$LRADBZ\n"
  ...
  plotWosymbol(adbz_wo,18,stSQAlt,"diamond",10,RED,0)
...
//    cloud_title = spat(fstem,"vfe",">")

    cloud_title = "passing_clouds"

<<"%V$cloud_title\n"

    cw= CreateGwindow(@title,"CW_$cloud_title")

    SetGwindow(cw,@resize,0.01,0.01,0.95,0.14,0)
    SetGwindow(cw,@drawon,@pixmapon,@save,@sticky,1,@fname,"$cloud_title")
...
/////////////////////////////////////////////////////////////////////////////////////////////////
   ...
   setgwob(lrip_wo,@drawon,@pixmapon,@save,@clip,0.1,0.1,0.98,0.98,@scales,0,0,185,120,@savescales,0)
...
///////////////////////////////////  Glines  /////////////////////////////////////////////////////
   ...
   srdbz_gl = CreateGline(@wid,cdbz_wo,@type,"Y",@yvec,LRVEC,@color,"red")
   lrdbz_gl = CreateGline(@wid,lrdbz_wo,@type,"Y",@yvec,LRVEC,@color,"cyan")
   strmtop_gl = CreateGline(@wid,lrdbz_wo,@type,"XY",@yvec,Strm_t,@xvec,Strm_r,@color,"green",@ltype,"symbol","triangle",@symsize,7)
   strmtopSQ_gl = CreateGline(@wid,lrdbz_wo,@type,"XY",@yvec,StrmSQ_t,@xvec,Strm_r)

   mp_gl = CreateGline(@wid,lri_wo,@type,"Y",@yvec,mc_alt,@color,"pink")
   setGline( mp_gl,@ltype,"symbol","diamond",@symsize,3,@symhue,RED)


   setGline( strmtopSQ_gl,@color,"red",@ltype,"symbol","diamond",@symsize,7,@symhue,RED)

   strmmidSQ_gl = CreateGline(@wid,lrdbz_wo,@type,"XY",@yvec,StrmSQ_m,@xvec,Strm_r)
   setGline( strmmidSQ_gl,@color,"red",@ltype,"symbol","cross",@symsize,4,@symhue,YELLOW)

   strmcbaseSQ_gl = CreateGline(@wid,lrdbz_wo,@type,"XY",@yvec,StrmSQ_cbase,@xvec,Strm_r)
   setGline( strmcbaseSQ_gl,@color,"red",@ltype,"symbol","star",@symsize,4,@symhue,GREEN)
