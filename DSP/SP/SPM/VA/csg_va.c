...
/* float tar_spec[256*512],tpl_spec[256*256]; */
...
/*   LOAD TO VA   */
   Sum = 0;
   Tmpl = 2;
   Targ = Tmpl + (n_tpl_frames * num_fq_bins) + 4;
    dbprintf(1,"Targ %d \n",Targ);
   
  
   idx = maplodfv ( tpl_spec , 4, Tmpl, 1, (n_tpl_frames * num_fq_bins));
   mapbwaitpkt (idx);
  dbprintf(1,"VA LOADED TMPL  \n");
   idx = maplodfv (&tar_spec[c2di(0,0)], 4, Targ, 1, (n_tar_frames * num_fq_bins));
   mapbwaitpkt (idx);
  dbprintf(1,"VA LOADED TARG \n");
...
/*   DOT PRODUCT */
	...
	for ( ioff = 0 ; ioff < n_tar_frames - n_tpl_frames ; ioff++ ) {
	 ...
	 dbprintf (1,"VA DOTP %d\n",ioff);
	idx = mapdotfv(Tmpl,1,Targ+(ioff * num_fq_bins),1,Sum, (n_tpl_frames * num_fq_bins));
	mapsync(idx);
	
           idx = mapstrfv (Sum, 1, &corr[t], 4, 1);
           mapbwaitpkt (idx);
