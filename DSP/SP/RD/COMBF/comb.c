...
dbprintf(1,"in %s  out %s\n",i_chn[0].dtype,o_df.dtype);
 ...
 /* sample frequency now is down by two*/
 		sf = sf/2.0; 
dbprintf(1,"sf now %f \n",sf);    
...
dbprintf(1,"read %d \n",eof);
	  ...
	  for (k = 1 ; k < eof/2 ; k++) {
	   ...
	   eof = fwrite(Sbuf,sizeof(short), eof/2, fp);
...
sho_use()
{
    fprintf(stderr," down sampler\n");
    fprintf(stderr,"-t throw away every n point [default 2]\n");
