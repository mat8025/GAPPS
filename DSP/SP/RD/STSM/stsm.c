...
/* where is f2 peak */
	...
	dbprintf(2,"F2 @ ame fpk2 @ %d %f\n",fpk2,f2);

	dbprintf(1,"F1 @ %f F2 @ %f\n",f1,f2);
	printf("%f %f %f\n",f1,f2,tot_ame);
	if (f2 > 800 && f2 < 2500.0 && tot_ame > 5 ) {
...
dbprintf ( 1,"loop %d sb %d new_sample %d asw_fn %d\n",loop,sb,new_sample,asw_fn);
