...
<<"%V $lcpx $wymin $wymax \n"
  //sGl(lc_gl,@cursor,lcpx,wymin,lcpx,wymax,@draw);
        sGl(lc_gl,@cursor,lcpx,wymin,lcpx,wymax);
        }
...
<<"%V $rcpx $wymin $wymax \n"
  //sGl(rc_gl,@cursor,rcpx,wymin,rcpx,wymax,@draw);
       sGl(rc_gl,@cursor,rcpx,wymin,rcpx,wymax);
       }
...
//  CURSORS
 ...
 ysc = 1.0
  ...
  // zinwo=cWo(aw,@name,"ZIN",@color,"hotpink")
  zinwo=cWo(aw,@ONOFF,@name,"ZIN",@color,"hotpink",@callback,"Zin")
...
int button = 0
   ...
   drawScreens()
    ...
    include "gevent.asl";   // examines events
   ...
   while (1) {
    ...
    if ( (ev_keyw @= "REDRAW") || (ev_keyw @= "RESIZE") || (ev_keyw @= "RESCALE") || (ev_keyw @= "PRINT")) {
	 ...
	 <<"$ev_woproc \n"
       ...
       if (!(ev_keyw @= "")) {
           DBPR"calling function via keyword |${ev_keyw}| $(typeof(ev_keyw))\n"
	   $ev_keyw();        
         }
