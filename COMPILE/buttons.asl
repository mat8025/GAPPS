...
//////// Wob //////////////////
 ...
 two=cWo(txtwin,@TEXT,@name,"Text",@VALUE,"howdy",@color,ORANGE_,@resize_fr,0.1,0.1,0.9,0.9,@flush)

 sWo(two,@BORDER,@DRAWON,@CLIPBORDER,@FONTHUE,"black",@pixmapoff,@redraw,@flush)
 sWo(two,@SCALES,-1,-1,1,1,@flush)
 sWo(two,@help," Mouse & Key Info ",@flush)

 gwo=cWo(vp,@BV,@name,"ColorTeal",@color,GREEN_,@resize,bx,by,bX,bY,@flush)
 
 sWo(gwo,@border,@drawon,@clipborder,@fonthue,RED_,@VALUE,"color is teal",@STYLE,SVB_,@flush)
 sWo(gwo,@bhue,TEAL_,@clipbhue,"skyblue",@redraw,@flush )

 bY = by - ypad
 by = bY - yht
 
 hwo=cWo(vp,@ONOFF,@name,"ENGINE",@VALUE,"ON",@color,RED_,@resize,bx,by,bX,bY,@flush)

 sWo(hwo,WBORDER_,WDRAWON_,WCLIPBORDER_,WSTYLE_,SVR_,EO_);
 //sWo(hwo,@fonthue,WHITE_, @STYLE,SVR_)
 sWo(hwo,WFHUE_,LILAC_,WBHUE_,BLUE_,EO_);
 sWo(hwo,@clipbhue,MAGENTA_);


 // GetValue after entering text
 gvwo=cWo(vp,@BV,@name,"GMYVAL",@VALUE,0,@color,GREEN_,@resize,0.5,by,0.9,bY,@flush);
 sWo(gvwo,@BORDER,@DRAWON,@CLIPBORDER,@FONTHUE,BLACK_, @STYLE,SVR_,@flush);
 sWo(gvwo,@bhue,"white",@clipbhue,"red",@FUNC,"inputValue",@MESSAGE,1,@flush);


 bY = by - ypad
 by = bY - yht

 lwo=cWo(vp,@ONOFF,@name,"PLAY",@VALUE,"ON",@color,RED_,@resize,bx,by,bX*0.5,bY,@flush);
...
<<"%V$two $hwo $gwo $gvwo $lwo\n"
 ...
 rwo=cWo(vp2,@BS,@name,"FRUIT",@color,"yellow",@resize,bx,by,bX,bY,@flush);
 sWo(rwo,@CSV,"mango,cherry,apple,banana,orange,Peach,pear",@flush);

 sWo(rwo,@BORDER,@DRAWON,@CLIPBORDER,@FONTHUE,"red",@STYLE,SVR_, @redraw ,@flush);
 sWo(rwo,@fhue,"orange",@clipbhue,"steelblue",@flush);

 boatwo=cWo(vp3,@BS,@name,"BOATS",@color,"yellow",@resize_fr,bx,by,bX,bY,@flush);
 sWo(boatwo,@CSV,"sloop,yacht,catamaran,cruiser,trawler,ketch",@flush)
 sWo(boatwo,@BORDER,@DRAWON,@CLIPBORDER,@FONTHUE,RED_,@STYLE,SVR_, @redraw)
 sWo(boatwo,@help," click to choose a boat ",@flush)
 bY = by - ypad
 by = bY - yht

<<"%V$boatwo \n"

 bsketchwo=cWo(vp3,@GRAPH,@name,"sketch",@color,"yellow",@resize,bx,0.1,0.9,bY,@flush);
 sWo(bsketchwo,@BORDER,@DRAWON,@CLIPBORDER,@FONTHUE,"red", @redraw )
 sWo(bsketchwo,@clip,0.1,0.15,0.95,0.85,@bhue,"lime")
 sWo(bsketchwo,@SCALES,-1,-1,1,1,@flush)

<<"%V$bsketchwo \n"


 grwo=cWo(vp2,@GRAPH,@name,"pic",@color,YELLOW_,@resize,bx,by,bX,bY,@flush);
 sWo(grwo,@BORDER,@DRAWON,@CLIPBORDER,@FONTHUE,RED_, @redraw )
 sWo(grwo,@SCALES,0,0,1,1,@flush);
...
<<"%V$grwo \n"
 ...
 qwo=cWo(vp2,@BN,@name,"QUIT",@VALUE,"QUIT",@color,MAGENTA_,@resize_fr,bx,by,bX,bY,@flush);
 sWo(qwo,@help," click to quit",@flush)
...
<<"have win interup -- exiting!\n"
      ...
      sWo(two,@texthue,"black",@clear,@textr,"$_ekeyw",-0.9,0,@flush);
/*
      if (_ename == "PRESS") {
	    <<"%V $_ewoname \n";
