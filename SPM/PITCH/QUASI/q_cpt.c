...
q_cep(in_buf,real,imag,fft_size,scale_factor,win_length,window,power,pow_thres)
float in_buf[];
float real[];
float imag[];
float window[];
...
/*	dbprintf(1,"%d %f %f\n",j,real[j],window[j]);  */
	...
	*power = 0.0;
	...
	if (*power > 0.0)
	*power =   10 * log10( *power/ (float) (fft_size/2) );
		else
	*power = -120.0;
...
q_pickpeaks(pam,results,cep_thres,inc_thres,n_peaks,start_at,finish_at)
float pam[];
float results[];
float cep_thres,inc_thres;
int n_peaks,start_at,finish_at;
{
	float wt_thres;
	int pks_found = 0;
	int step_isp = -1;
...
float *old_pp,*rave_pp;
{
	...
	*old_pp = pitch_stats[1];
	...
	if (npm >= 15) {
		rppi = npm - 15;
		sp_stats(&the_pitch[rppi],pitch_stats,15);
	}

	*rave_pp =pitch_stats[1];
dbprintf(1,"doing stats npm %d ave %f rave %f\n",npm,*old_pp,*rave_pp);
}
...
dbprintf(1,"POSSIBLE reject %d  %d rejects allowed\n",n_reject,a_reject);
	
	        if (the_amp < 2.0) {
	dbprintf(1,"REJECT by AMP new_pp %f rave_pp %f the_amp %f \n",new_pp,rave_pp,the_amp);	        	
		   return 1;
	        }

		if ( n_reject < a_reject ) { /* reject  ? */
	dbprintf(1,"REJECT new_pp %f rave_pp %f the_amp %f \n",new_pp,rave_pp,the_amp);
			return 1;
		}
	}
	return 0;
...
float results[];
float the_pitch[];
float power,pow_thres,min_pow_thres,dt;
int n_peaks;
{
	static int npm = 0;
	float old_pp,rave_pp;
...
dbprintf(1,"in pitch track power %f pow_thres %f pk_amp %f\n",power,p_thres,results[n_peaks]);
...
dbprintf(1,"Hz1 %f Hz2 %f ave_hz %f rave %f npm %d \n", pitch0, pitch1,old_pp, rave_pp, npm );

			if (  doubling ) {

dbprintf(1,"possible doubling or subharmonic chose closest to ave pitch\n");
			...
			else if  (  (dp1 < dp0)  && (results[n_peaks] < (3.0 * results[n_peaks + 1]) )) { /* chose closest to rave if amps similar */
				q_set_pitch(results,1,n_peaks,dt,&new_pp,&the_lag,&the_amp);
				dbprintf(1,"chose %f closest to ave %f  npm %d\n",new_pp,rave_pp,npm);
					alt_pp = pitch0;
					alt_pp_amp = results[n_peaks];
			}
				...
				q_set_pitch(results,0,n_peaks,dt,&new_pp,&the_lag,&the_amp);
				dbprintf(1,"chose strongest %f  npm %d\n",new_pp, npm);
					alt_pp = pitch1;
					alt_pp_amp = results[n_peaks+1];
			}
		}
...
dbprintf(1,"THE_HZ is %f lag %f pk_amp %f  npm %d  alt is %f altamp %f\n",results[0], results[1],results[3],npm,alt_pp,alt_pp_amp);
