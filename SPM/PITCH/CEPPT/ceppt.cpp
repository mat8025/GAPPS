...
//                           ceppt.cpp 
//		          
//    @comment  computes pitch track from vox file -updated 
//    @release   Beryllium  
//    @vers 1.3 Li Lithium 6.4.76 C-Be-Os 
//    @date 02/04/2023 15:35:50    
//    @cdate 01/29/2000          
//    @author: Mark Terry                                  
...
//    - 
///////////////////////////////////<v_&_v>//////////////////////////////////*/ 
static char     gaspid[] = "ceppt.cpp     @vers 1.2 He Helium  Sat Feb  4 13:46:17 2023 Mark Terry ";
//>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>//
...
float choose_one(float hz0,float  hz1,float  ave_pp,float  rave_pp,float  last_pp,float  contin,float  accept_hz_delta)
...
float track_hz(float hz,float  ave_pp,float  rave_pp,float  last_pp,float  accept_hz_delta,float  contin)
...
void adjust_hz_delta(float          *accept_hz_delta,float           ave_pp,int contin)
{
...
void fill_hole( float           pa[], int spi, int loop)
{
	float           hz;
	hz = pa[spi];
...
void btg(float buffer[], int k, int span)
{
	float           hz;
	float           hz1, hz2;
	int             i, j;
...
int compute_ener_zx(int    nfpc,	float           frame_shift,
			float           sf,
			int             ener_win)
{
	int             loop = 0;
...
int write_tracks(int  loop,float  *the_pp,float   min_pow_thres, data_file   *o_spt)
{
	int             spi;
	float           new_pp;
	int             npm = 0;
...
void smooth_and_fill(int   loop,	float           max_pitch_gap,float           frame_shift)
{
	int             span;
	int             spi;
...
void track_2stage(int loop, 	float           ave_pp,	float          *last_pp)
{
...
void setup_pitch_df(	char           *spp_file)
{
	int             posn,i;
        channel n_chn;
	...
	o_spt.f[SF] = sf;
	o_spt.f[DF_STR] = i_df.f[DF_STR];
	o_spt.f[DF_STP] = i_df.f[DF_STP];
	o_spt.f[CN] = 5;
...
void setup_lag_df(char           *pt_file)
{
	int             posn;
	int             ch_num = 0;
	...
	o_pt.f[SF] = i_df.f[SF];
	o_pt.f[DF_STR] = i_df.f[DF_STR];
	o_pt.f[DF_STP] = i_df.f[DF_STP];
	o_pt.f[CN] = 4;
...
void setup_cep_df(char *out_file,int n_lag, float frame_shift, float frame_length)
...
void setup_ph_df(char *ph_file, float max_pitch, int frame_shift, int frame_length)
...
int read_chunk(int the_chunk, int n_chunks, int chunk_sec,
	       int the_samples, int nsamples, int ener_win, float *off_time, int *vp)
{
	static int      keep_s = 0;
	int             ii,jj;
	long int        fposn;
...
void set_pow_thres(int loop, int the_chunk)
{
	float           start_value;
...
int compute_chunk(	int  *n_chunks,  int *chunk_sec, int nsamples)
{
	int             the_samples;
...
void update_ph(int loop, 	float           power,	float           ph_interval)
{
	int             spi;
