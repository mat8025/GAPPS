C-**********************************************************************
C-*                       SPALIB.FOR                                   *
C-*            SUBPROGRAMS IN ALPHABETICAL ORDER                       *
C-*                      IN FORTRAN-77                                 *
C-*                                                                    *
C-**********************************************************************
C-
...
C-FFT LENGTH, N, USED INTERNALLY, IS N = L-1.
C-LET K=INDEX OF LAST NONZERO SAMPLE IN Y(0)---Y(N-1).  THEN X(0)
C- THRU X(N-1) MUST INCLUDE PADDING OF AT LEAST K-NMIN ZEROS.
...
C-       3  LY NOT A POWER OF 2.
      REAL X1(0:LX),X2(0:LX)
      COMPLEX Y(0:LY),WORK(0:2*LY-1),P,Q,U,V
      IERROR=1
    ...
    4   CONTINUE
        ...
        DO 5 M=1,LY
          P=WORK(M)+WORK(2*LY-M)
          Q=WORK(M)-WORK(2*LY-M)
          U=.5*CMPLX(REAL(P),AIMAG(Q))
          V=.5*CMPLX(AIMAG(P),-REAL(Q))
          Y(M)=Y(M)+CONJG(U)*V/(TSV*NSGMTS)
    5   CONTINUE
    6 CONTINUE
      IERROR=0
    7 RETURN
...
C-  Y((N-1)/2) IF N ODD, ARE THE COMPLEX DFT COMPONENTS, WITH
C-  COSINE COMPONENT=REAL PART AND SINE COMPONENT=IMAGINARY PART.
C-Y AND X CANNOT BE THE SAME ARRAY.
      DIMENSION X(0:N-1)
...
C-IERROR=0      NO ERRORS DETECTED
C-       1      INVALID L -- <0
C-       2      AAVECT(0) TOO SMALL -- <1.E-10
C-       3      SINGULAR CONDITION: SCL DROPS >5 ORDERS OF MAGNITUDE
C-                                  OR RMS ERROR DECREASES BY <1%
    ...
    2   CONTINUE
        SCL=AAVECT(0)-GAMMAP
        IF(ABS(SCL/AAVECT(0)).LT.1.E-5) GO TO 6
    ...
    3   CONTINUE
     ...
     +            B((NN-1)/2)=B((NN-1)/2)+BETA*B((NN-1)/2)
        ...
        DO 4 I=1,NN
         S=S-B(I)*AAVECT(I+1)
    4   CONTINUE
        IF(S0.LE.1.E-10 .AND. S.LE.1.E-10) GO TO 6
        IF(S0.GT.1.E-10 .AND. (S-S0)/S0.LT.1.E-4) GO TO 6
        S0=S
    5 CONTINUE
      IERROR=0
    6 DO 7 I=0,L
...
C-              1    ALL ZERO TRANSFER FUNCTION
C-              2    SPBILN: INVALID TRANSFER FUNCTION
...
C-COMPUTATION IS IN PLACE, OUTPUT REPLACES INPUT.
C-INPUT:  REAL VECTOR X(0:N+1) WITH REAL DATA SEQUENCE IN FIRST N
C-        ELEMENTS; ANYTHING IN LAST 2.  NOTE: X MAY BE DECLARED
C-        REAL IN MAIN PROGRAM PROVIDED THIS ROUTINE IS COMPILED 
C-        SEPARATELY ... COMPLEX OUTPUT REPLACES REAL INPUT HERE.
C-OUTPUT: COMPLEX VECTOR XX(O:N/2), SUCH THAT X(0)=REAL(XX(0)),X(1)=
C-        IMAG(XX(0)), X(2)=REAL(XX(1)), ..., X(N+1)=IMAG(XX(N/2).
...
C-L=FILTER SIZE SUCH THAT F(Z) = B(0) + B(1)*Z**(-1) +...+ B(L)*Z**(-L)
...
C-       1     INVALID L<0
C-       2     AVECT(0)<1.E-10
C-       3     SINGULAR CONDITION: V DROPS >5 ORDERS OF MAGNITUDE
C-                                 OR RMS ERROR DECREASES BY <1%
    ...
    3   CONTINUE
       ...
       DO 4 K=0,M
        ...
        S=S+B(K)*C(K)
    4  CONTINUE
       IF(S0.LE.1.E-10 .AND. S.LE.1.E-10) RETURN
       IF(S0.GT.1.E-10 .AND. (S-S0)/S0.LT.1.E-4) RETURN
       S0=S
    5 CONTINUE
      IERROR=0
      RETURN
...
C-IERROR=0      NO ERRORS DETECTED
C-       1      N<=0 
C-       2      ARITHMETIC PROBLEM - DENOMINATOR APPROX 0.
      DIMENSION X(0:N-1),Y(0:N-1)
      A=0.
      B=0.
      IERROR=1
...
C-LATEST DATE: 10/26/90
C-GENERATES MATRIX USED FOR ORTHOGONAL POLYNOMIALS
      DIMENSION P(0:L,0:L)
      DO 2 I=0,L
        P(I,0)=1.
        DO 1 J=I+1,L
...
C-LATEST DATE: 10/15/90
C-CONVERTS TRANSFER FUNCTION COEF. FROM DIRECT TO LATTICE FORM.
...
C-       1     UNSTABLE H(Z)
C-WORK ARRAY IS WORK(0:L,3) AND IS USED INTERNALLY.
      DIMENSION B(0:L),A(L),WORK(0:L,3)
      REAL KAPPA(0:L-1),NU(0:L)
      IERROR=1
      WORK(0,1)=1.
      WORK(0,3)=B(0)
    ...
    3   CONTINUE
        NU(LL)=WORK(LL,3)
        DO 4 J=0,LL
          WORK(J,3)=WORK(J,3)-NU(LL)*WORK(J,2)
    4   CONTINUE
    5 CONTINUE
      NU(0)=WORK(0,3)
      IERROR=0
...
C-**********************************************************************
C-  
      SUBROUTINE SPSOLV(A,C,L,B,IERROR)
C-LATEST DATE: 10/10/90
...
C-COMPUTES VARIANCE OF N-POINT DATA VECTOR X(0:N-1)
      ...
      DO 1 I=0,N-1
        DSUM=DSUM+X(I)**2
    1 CONTINUE
      SPVARI=MAX(0.,REAL((DSUM-N*SPMEAN(X,N)**2)/(N-1.)))
      RETURN
      END
C-
C-**********************************************************************
...
C-    X(LX) CAN BE ANYTHING INITIALLY.)  COMPUTATION IS IN PLACE.
