 ...
 *			PK_VAL							 *
 	...
 	a spectrum file 
        and locate the peaks and/or dips
 *      Mark Terry 1992
 *										 *
...
/* write channel headers */
   ...
   for (i = 0 ; i < n_peaks + n_vals ; i++, ch_num++) {
...
if (m_ave) {
  ...
  Results[pk] = (pk_pos + Mave[pk][0] + Mave[pk][1]) / 3.0;
			 Mave[pk][0] = Mave[pk][1];
			 Mave[pk][1] = pk_pos ;

			 pka = pk+n_peaks;

  Results[pka] = (pk_amp + Mave[pka][0] + Mave[pka][1]) / 3.0;
			 Mave[pka][0] = Mave[pka][1];
			 Mave[pka][1] = pk_amp ;

			 pkb = pk+2*n_peaks;
  Results[pkb] = (pk_bw + Mave[pkb][0] + Mave[pkb][1]) / 3.0;
			 Mave[pkb][0] = Mave[pkb][1];
			 Mave[pkb][1] = pk_bw ;

}
else {
	Results[pk] = pk_pos;
	Results[pk+n_peaks] = pk_amp;	        
...
if (peaks_dips_only)
fwrite (Results, sizeof(float), n_peaks + n_vals, o_df.fp);
else
fwrite (Results, sizeof(float), 3 * (n_peaks + n_vals), o_df.fp);
