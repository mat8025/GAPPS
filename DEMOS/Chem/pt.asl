...
//<<" %v $spawn_it \n"
    ...
    vp = cWi(@title,"Periodic_Table_Of_Elements",@resize,0.01,0.1,0.99,0.95,0)

    sWi(vp,@pixmapon,@drawon,@save,@bhue,WHITE_)
    sWo(vp,@grid,11,20)
    sWi(vp,@clip,0.01,0.2,0.99,0.8)
...
//////// Wob //////////////////
 ...
 ewo[ia]=cWo(vp,"BV",@name,"$elef[1]  $elef[2]",@color,ecolor[ia],@resize,col,rb,col+1,rt,3)
  ...
  sWo(ewo[ia],@BORDER,@DRAWON,@CLIPBORDER,@fonthue,BLACK_,@font,"italic",@VALUE,"$elef[0]\n $elef[3]",@STYLE,"SVB")
...
//======================================//
 ...
 int i = 0;
  for (i = si ; i <=fi; i++) {
   ...
   sWo(ewo[i],@BORDER,@DRAWON,@CLIPBORDER,@FONTHUE,BLUE_,@font,"tiny",@VALUE,"$elef[0]\n $elef[3]",@STYLE,"SVB")
