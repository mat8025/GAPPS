...
// how about elev ??
   ...
   setgwob(vptxt,@scrollclip,UP,8,@textr,"$_proc %V$obpz $obpx ",0,-0.25) 
...
<<[CFH]" in $_proc \n"
     ...
     //setgwob(vptxt,@scrollclip,UP,8,@textr,"PlanView ",0,-0.2) 
...
{
<<[CFH]" in $_proc \n"
     //setgwob(vptxt,@scrollclip,UP,8,@textr,"SideView ",0,-0.2) 
...
<<" read scene \n"
   ...
   obid1 = MakeObject("G_CUBE",0,0,50,1,1,1,50)
...
///////////////////////// SETUP WINDOWS AND WOBJS //////////////////////////////
  ...
  setgwob(vptxt,@BORDER,@DRAWON,@CLIPBORDER,@FONTHUE,"black", @redraw, @pixmapon, @drawon)

  setgwob(vptxt,@scales,-1,-1,1,1)

  vpwo=CreateGWOB(vp,"GRAPH",@name,"VP",@resize,0.2,0.2,0.8,0.90,@color,"white")
  // pixmap draw error?
  setgwob(vpwo,@scales,-20,-20,20,20, @save,@redraw,@drawoff,@pixmapoff,@drawon)


  pvwo = CreateGWOB(vp,"GRAPH",@resize,0.01,0.11,0.19,0.5,"name","PLANVIEW","color","white")

  setgwob(pvwo,@scales,-200,-200,200,200, @save,@redraw,@drawoff,@pixmapon,@drawon)

  svwo = CreateGWOB(vp,"GRAPH",@resize,0.01,0.51,0.19,0.95,@name,"SIDEVIEW","color","white")

  SetGwob(svwo,@scales,-200,-200,200,200, @save,@redraw,@drawoff,@pixmapon, @drawon)
...
<<" in $_proc checkRotate $wc \n"
     ...
     case 'Q':
          rotate_vec(-2)
          setgwob(vptxt,@scrollclip,UP,8,@textr,"R rotate ",0,-0.2) 
          ret = 1
      break;
     ...
     case 'S':
        <<" rotate S \n"
          rotate_vec(2)
          setgwob(vptxt,@scrollclip,UP,8,@textr,"S rotate ",0,-0.2) 
          ret = 1
      break;
     }
...
//<<" in $_proc checkRoll $wc \n"
    ...
    switch (wc) {
       case '[':
          roll(-2)
          ret = 1
          setgwob(vptxt,@scrollclip,UP,8,@textr,"[ roll left",0,-0.25) 
       break;
       case ']':
          roll(2)
          ret = 1
          setgwob(vptxt,@scrollclip,UP,8,@textr,"] roll right",0,-0.25) 
          break;
      }
...
//<<" in $_proc checkMove $wc \n"
    ...
    switch (wc) {
        ...
        move_vec(2.0)
        ret = 1
        setgwob(vptxt,@scrollclip,UP,8,@textr,"$E->nm R move forward ",0,-0.25) 
        break;
        ...
        move_vec(-2.0)
        ret = 1
        setgwob(vptxt,@scrollclip,UP,8,@textr,"$E->nm T move back ",0,-0.25) 
        break;
      }
...
//<<" in $_proc checkObserver $wc \n"
    ...
    switch (wc) {
     ...
     case 'X':
      ...
      if (ret == 1) {
          setgwob(vptxt,@scrollclip,UP,8,@textr,"$wc  Observer posn  ",0,-0.2) 
      }
...
proc checkKeyCommands(char wc)
{
  ret = 1
<<[CFH]" in $_proc $wc \n"
//    setgwob(vptxt,@scrollclip,UP,8,@textr,"$_proc $wc",0,-0.25) 

    switch (wc) {

      case 'H':
          MoveObject(wobj,0,2,0)
        setgwob(vptxt,@scrollclip,UP,8,@textr,"MoveObject",0,-0.25) 
        break;
      case 'h':
          MoveObject(wobj,0,-2,0)
        break;
      ...
      case 'g':
          GridON = !GridON
        setgwob(vptxt,@scrollclip,UP,8,@textr,"Grid $GridON",0,-0.25) 
        break;
      case 'f':
          <<[CFH]"yep got go forward \n"
        setgwob(vptxt,@scrollclip,UP,8,@textr,"forward ",0,-0.25) 
                  go_on = 1
                  o_speed = 2.0
                  go_rotate = 0
                  go_circle = 0
      ...
      case 'p':
          ...
          rotate_vec(5)
          go_circle = 0
        setgwob(vptxt,@scrollclip,UP,8,@textr,"p rotate ",0,-0.25) 
        break;
      case 'o':
          go_rotate = 1
          rotate_vec(speed)
          setgwob(vptxt,@scrollclip,UP,8,@textr,"o rotate ",0,-0.25) 
          go_circle = 0
        break;
      case '//':
       resetobs(2)
      ...
      case '.':
         ...
         go_loop = 0
         go_circle = 0
         setgwob(vptxt,@scrollclip,UP,8,@textr,". stop ",0,-0.25) 
//       resetobs(3)
        break;
      case ',':
     // make observer postion center of map
        setgwob(vptxt,@scrollclip,UP,8,@textr,"center observer ",0,-0.25) 
        center_map()  
        break;
      case '?':
       resetobs(5)
...
//       resetobs(6)
      ...
      case 'd':
       distance *= 0.9
      setgwob(vptxt,@scrollclip,UP,8,@textr,"distance $distance ",0,-0.25) 
        break;
      case 'D':
       distance *= 1.1
      setgwob(vptxt,@scrollclip,UP,8,@textr,"distance $distance ",0,-0.25) 
...
//        break;
      case 'a':   
       ...
       setgwob(vptxt,@scrollclip,UP,8,@textr,"a rotate object ",0,-0.25) 
            break;
      case 's':   
        RotateObject(wobj,0,0,-2.0)
        setgwob(vptxt,@scrollclip,UP,8,@textr,"s rotate object ",0,-0.25) 
            break;
      case 'q':     // rotate obj 0
        RotateObject(wobj,4,0,0.0)
        setgwob(vptxt,@scrollclip,UP,8,@textr,"q rotate object ",0,-0.25) 
            break;
      case 'w':     // rotate obj 0   
        RotateObject(wobj,-2,0,0.0)
        setgwob(vptxt,@scrollclip,UP,8,@textr,"w rotate object ",0,-0.25) 
            break;
      case 'e':     // rotate obj 0
        RotateObject(wobj,0,4,0.0)
        setgwob(vptxt,@scrollclip,UP,8,@textr,"e rotate object ",0,-0.25) 
            break;
      case 'r':   
       // rotate obj 0
        RotateObject(wobj,0,-2,0.0)
        setgwob(vptxt,@scrollclip,UP,8,@textr,"r rotate object ",0,-0.25) 
            break;
      case '\'':   
        resetobs(1)
        setgwob(vptxt,@scrollclip,UP,8,@textr,"%c$wc reset obs ",0,-0.25) 
            break;
      default:
           ret = 0;
      }
...
setgwob(vptxt,@scrollclip,UP,8,@textr,"%$ac %V$elev ",0,-0.25) 
       break;
      case 'I':
          elev += 2
          if (elev > 90) {
...
<<"at xgs interact ! $ml\n"
   ...
   setgwob(vptxt,@scrollclip,UP,8,@clipborder,"red",@textr,E->emsg,0,0.8) 
...
// setgwob(vptxt,@scrollclip,UP,8,@textr,"$E->nm $E->keyw %c$E->keyc %$E->woname",0,0.1) 
      ...
      if (Evtype @= "PRESS") {
         ...
         // setgwob(vptxt,@scrollclip,UP,8,@clipborder,"blue",@textr,E->kws[2],0,0) 
 ...
 if (did_cont ) {
   ...
   if (elev < -90) {
    elev = -90
       ...
       setgwob(azimwo,@VALUE, "%5.1f$azim" , @update)
       setgwob(distwo,@VALUE, "%5.1f$distance" , @update)
       setgwob(elevwo,@VALUE, "%5.1f$elev" , @update)
...
//   setgwob(two,@text,txtmsg, @update) //wrong woid  causes crash - should be robust

     setgwob(vptxt,@text,txtmsg,@update) 
