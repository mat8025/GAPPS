...
// usage
// asl viewWgrid.asl [grid/terrain file ]  [objects_files]
// asl viewWgrid.asl NED/NED_04312054  scene
...
// observer position is decimal degrees ?
...
// ned_location ---    /home/mark/GLIDE/NED/
// drwxrwxr-x 2 mark mark     4096 2009-09-24 10:46 NED_04312054
// drwxrwxr-x 2 mark mark     4096 2009-09-24 10:46 NED_51067233

ned_stem = _clarg[1] // this is a must have --- to read in NED -- 1 arc second approx 30 m resolution

if ( ned_stem @= "" ) {

  ned_stem = "NED/NED_04312054"  // Colorado mtns
...
//i_read()
//  now get a subsection of Map --- for Matrix Object

// use a grid -- instead of showTerrain !
...
//  get a subset of the Wgrid for display as a Matrix Object!
   ...
   SG[0:(res-1)][::] = Map[offr:er:gstep][offc:ec:gstep]
...
proc center_map()
{
     ...
     SetGwob(mapwo,"scales",rx,ry,rX,rY)
}

proc map_home()
{
// W left E right on axis
     rx = MidLng - mapscale
     rX = MidLng + mapscale
...
proc PlanView()
{
     ...
     plot3D(pvwo,scene,obpx,obpy,obpz, azim, elev,distance,mode,do_fill,GridON, show_eye)

     xzvec = lnglat2xygrid(obpx,obpz, LongW, LatN)

<<"PV %V$LongW $LatN $obpx $obpz $xzvec \n"
     xpm = xzvec[0]
     zpm = xzvec[1]

     plotgw(pvwo,"symbol",xpm, zpm ,"diamond",5,"blue",0)
...
proc MapView()
{
     ...
     SetGwob(mapwo,@clearpixmap)

     plotgw(mapwo,"symbol",obpx,obpz,"diamond",10,"blue",0)
     plotgw(mapwo,"symbol",obpx,obpz,"arrow",15,"blue",cd2pol(azim)-90,1)
     plotgw(mapwo,"symbol",targ_x,targ_z,"tri",7,"red",0)

     plotbox(mapwo, LongW, LatS, LongE, LatN ,BLUE)


/{
     plotline(mapwo, LongW, LatS, LongE, LatN ,BLUE)
     plotline(mapwo, LongW, LatN, LongE, LatS ,RED)
     plotline(mapwo, LongW, LatN, LongE, LatN ,BLUE)
     plotline(mapwo, LongW, LatN, LongW, LatS ,GREEN)
/}

     Setgwob(mapwo,@hue,"black",@showpixmap,@clipborder,@save)
...
//  we know we have to have some objects 
  ...
  //obid1 = MakeObject("MATRIX",SG,0,0,0,100,1.0,100,1,0,0,0,1)
...
///////////////////////// SETUP WINDOWS AND WOBJS //////////////////////////////
  ...
  float PV_dist = 400000.0;

  vp = CreateGwindow(@title,"vp",@resize,0.1,0.01,0.98,0.98,0)

  SetGwindow(vp,"scales",-2000,-20,2000,50,0, @drawoff,@pixmapon,@save,@bhue,"white")

  SetGwindow(vp,"clip",0.01,0.1,0.95,0.99)

 //gwo=createGWOB(vp,"BV",@name,"B_V",@color,"green",@resize,bx,by,bX,bY)

  vptxt=CreateGWOB(vp,"TEXT",@name,"TXT",@resize,0.4,0.01,0.75,0.1,@color,"blue")
  ...
  mapwo = CreateGWOB(vp,"GRAPH",@resize,0.01,0.05,0.19,0.35,"name","MAPVIEW","color","white")

  setgwob(mapwo, "scales", LongW, LatS, LongE, LatN )

  setgwob(mapwo,@save,@redraw,@drawoff,@pixmapon)

<<"mapwo to $LongW $LatS $LongE $LatN \n"

  svwo = CreateGWOB(vp,"GRAPH",@resize,0.01,0.71,0.19,0.95,@name,"SIDEVIEW","color","white")

  SetGwob(svwo,@scales,LongW,-100,LongE,5000, @save,@redraw,@drawoff,@pixmapon)

  pvwo = CreateGWOB(vp,"GRAPH",@resize,0.01,0.36,0.19,0.70,@name,"PLANVIEW","color","white")

  SetGwob(pvwo,@scales,-1000,-PV_dist,PV_dist,1000, @save,@redraw,@drawoff,@pixmapon)
...
/////////////////////////////////////////////////// CONTROLS ////////////////////////////
 ...
 obxwo=createGWOB(vp,"BV",@name,"OBX",@VALUE,"1",@color,"white")
 setgwob(obxwo,@BORDER,@DRAWON,@CLIPBORDER,@FONTHUE,"black",@style,"SVB")

 obywo=createGWOB(vp,"BV",@name,"OBY",@VALUE,"1",@color,"white")
 setgwob(obywo,@BORDER,@DRAWON,@CLIPBORDER,@FONTHUE,"black",@style,"SVB")

 obzwo=createGWOB(vp,"BV",@name,"OBZ",@VALUE,"1",@color,"white")
 setgwob(obzwo,@BORDER,@DRAWON,@CLIPBORDER,@FONTHUE,"black",@style,"SVB")


 targxwo=createGWOB(vp,"BV",@name,"TGX",@VALUE,"1",@color,"white")
 setgwob(targxwo,@BORDER,@DRAWON,@CLIPBORDER,@FONTHUE,"black",@style,"SVB")

 targywo=createGWOB(vp,"BV",@name,"TGY",@VALUE,"1",@color,"white")
 setgwob(targywo,@BORDER,@DRAWON,@CLIPBORDER,@FONTHUE,"black",@style,"SVB")

 targzwo=createGWOB(vp,"BV",@name,"TGZ",@VALUE,"1",@color,"white")
 setgwob(targzwo,@BORDER,@DRAWON,@CLIPBORDER,@FONTHUE,"black",@style,"SVB")



 int conwos[] = {  azimwo, distwo, elevwo, obxwo, obywo, obzwo ,targxwo, targywo, targzwo} 

 wo_rctile(conwos,bx,0.2,bX,0.75,3,3)
 ...
 upwo=createGWOB(vp,SYMBOL,@name,"N",@IVALUE,WUP,@color,"green",@symbol,"tri")

 setgwob(upwo,@FONTHUE,"black")

 dwnwo=createGWOB(vp,SYMBOL,@name,"S",@IVALUE,WDOWN,@color,"green",@symbol,"i_tri")

 setgwob(dwnwo,@FONTHUE,"black")

  ltwo=createGWOB(vp,SYMBOL,@name,"W",@IVALUE,WLEFT,@color,"green",@symbol,"tri")

  setgwob(ltwo,@SYMHUE,"black", @symang,90)

 rtwo=createGWOB(vp,SYMBOL,@name,"E",@IVALUE,WRIGHT,@color,"green",@symbol)

  setgwob(rtwo,@FONTHUE,"black", @symang,-90)

 ruwo=createGWOB(vp,SYMBOL,@name,"NE",@IVALUE,WRUP,@color,"green",@symbol,"tri")

  setgwob(ruwo,@FONTHUE,"black",@symang,-45)

   rdwo=createGWOB(vp,SYMBOL,@name,"SE",@VALUE,"1",@IVALUE,WRDWN,@color,"green",@symbol)

  setgwob(rdwo,@FONTHUE,"black",@symang,-135)

   luwo=createGWOB(vp,SYMBOL,@name,"NW",@VALUE,"1",@IVALUE,WLUP,@color,"green",@symbol,"tri")

  setgwob(luwo,@FONTHUE,"black", @symang,45)

   ldwo=createGWOB(vp,SYMBOL,@name,"SW",@VALUE,"1",@IVALUE,WLDWN,@color,"green",@symbol,"tri")

  setgwob(ldwo,@FONTHUE,"black", @symang,135)

   cenwo=createGWOB(vp,SYMBOL,@name,"CEN",@VALUE,"1",@IVALUE, WCEN,@color,"orange",@symbol,"dia")

 setgwob(cenwo,@SYMHUE,"black", @symang,0)


  int position_array[] = { luwo, upwo, ruwo,  ltwo,  cenwo, rtwo, ldwo, dwnwo, rdwo }


    wo_rctile(position_array,bx, 0.05, bX, 0.2,3,3)
    setgwob(position_array,@BORDER,@DRAWON,@PIXMAPON,@CLIPBORDER)
    setgwob(position_array,@redraw)
...
//       resetobs(3)
        break;
      case ',':
     // make observer position center of map
        center_map()  
        break;
      case '?':
       resetobs(5)
...
//  what grid coors are we using here ?
//  lat,long DD's  to and XY grid in meters?
...
//<<"%V$E->keyw %c$E->keyc %d$E->keyc  $E->woname\n"
   ...
   <<[CFH]"$E->keyw  %V%c$E->keyc \n"
     ...
     setgwob(vptxt,@textr,"$E->nm $E->keyw %c$E->keyc %s$E->woname",0,0.1) 
...
<<"$g_elev \n"
       ...
       setgwob(elevwo,@VALUE, "%5.1f$elev" , @redraw)
       setgwob(obxwo,@VALUE, "%5.1f$obpx" , @redraw)
//       setgwob(obywo,@VALUE, "%5.1f$(obpy*m2ft)" , @redraw)
       setgwob(obywo,@VALUE, "%5.1f$obpy" , @redraw)
       setgwob(obzwo,@VALUE, "%5.1f$obpz" , @redraw)
       setgwob(targxwo,@VALUE, "%5.1f$targ_x" , @redraw)
       setgwob(targywo,@VALUE, "%5.1f$targ_y" , @redraw)
       setgwob(targzwo,@VALUE, "%5.1f$targ_z" , @redraw)
    ...
    plot3D(vpwo, scene, obpx, obpy, obpz, azim, elev,distance,mode,do_fill,GridON, show_eye)
...
<<"%V5.1f$obpx , $obpy , $obpz , $azim  $elev  $distance\n"
...
setgwob(vptxt,@clipborder,"red",@textr,"done plot",0,0.4) 
     sleep(0.01)
