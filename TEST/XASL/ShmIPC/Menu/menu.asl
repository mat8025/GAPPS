...
// make a window
    ...
    vp2 = CreateGwindow(@title,"DailyDiet")

    SetGwindow(vp2,@pixmapon,@drawoff,@save,@bhue,"white")

    SetGwindow(vp2,@clip,0.1,0.2,0.9,0.9)


// make a Wob 

 gwo=createGWOB(vp2,"MENU",@name,"When",@color,"green",@resize,0.1,0.8,0.2,0.9)
 setgwob(gwo,@BORDER,@DRAWON,@CLIPBORDER,@FONTHUE,"black",@VALUE,"When",@STYLE,"SVB")
 setgwob(gwo,@bhue,BLUE,@clipbhue,"skyblue")
 setgwob(gwo,@menu,"Breakfast,AM-Snack,Lunch,PM-Snack,Dinner,Supper,MN-Snack")

 gwo2=createGWOB(vp2,"MENU_FILE",@name,"FoodTypes",@color,"green",@resize,0.21,0.8,0.3,0.9)
 // does value remain or reset by menu?
 setgwob(gwo2,@BORDER,@DRAWON,@CLIPBORDER,@FONTHUE,"blue",@VALUE,"Meat",@STYLE,"SVB",@menu,"Food.m")
 setgwob(gwo2,@bhue,"red",@clipbhue,"skyblue")


 portwo=createGWOB(vp2,"MENU",@name,"Portion",@color,"green",@resize,0.31,0.8,0.4,0.9)
 setgwob(portwo,@BORDER,@DRAWON,@CLIPBORDER,@FONTHUE,"black",@VALUE,"When",@STYLE,"SVB")
 setgwob(portwo,@bhue,BLUE,@clipbhue,"skyblue")
 setgwob(portwo,@menu,"1,2,3,Tiny,Small,Medium,Large,Huge")


 querywo=createGWOB(vp2,"BN",@name,"QUERY",@color,"green",@resize,0.41,0.8,0.5,0.9)
 setgwob(querywo,@BORDER,@DRAWON,@CLIPBORDER,@FONTHUE,"black",@VALUE,"QUERY",@STYLE,"SVB")
 setgwob(querywo,@bhue,BLUE,@clipbhue,"skyblue")





 gwo3=createGWOB(vp2,"SHEET",@name,"DailyDiet",@color,"green",@resize,0.1,0.1,0.9,0.7)
 // does value remain or reset by menu?

 setgwob(gwo3,@BORDER,@DRAWON,@CLIPBORDER,@FONTHUE,"red",@VALUE,"SSWO")

 setgwob(gwo3,@bhue,"cyan",@clipbhue,"skyblue")
...
Event E
   ...
   while (1) {
        ...
        if (! (E->keyw @= "NO_MSG")) {
          <<"$E->keyw $E->woname $E->woval\n"

          if (E->keyw @= "When") {
             when = E->woval
             setgwob(gwo3,@sheetrow,1,0,"$when,$food,$portion,20,90,1,7,6")
             setgwob(gwo3,@redraw)
          }

          else if (E->keyw @= "Portion") {
             portion = E->woval
             setgwob(gwo3,@sheetrow,1,0,"$when,$food,$portion,20,90,1,7,6")
             setgwob(gwo3,@clear,@redraw)
          }

          else if (E->keyw @= "FoodTypes") {
             food = E->woval
             setgwob(gwo3,@sheetrow,1,0,"$when,$food,$portion,20,90,1,7,6")
             setgwob(gwo3,@redraw)
          }
          else if (E->woname @= "QUERY") {
